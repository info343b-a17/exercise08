...
import ChirpList from '../problem-bc/src/chirper/ChirpList' //individuals?
import ChirpBox from '../problem-bc/src/chirper/ChirpBox'
...
describe('Source code is valid', () => {
  test('JavaScript lints without errors', async () => {
    const sources = ['index.js','App.js', 'chirper/ChirpBox.js', 'chirper/ChirpList.js'].map((src) => JS_FILE_PATH_ROOT+src);
...
describe('The Chirper app', () => { 
  ...
  beforeAll(() => {
    db = firebase.database().ref(); //make the db    
  })
  ...
  it('should post messages from the ChirpBox', () => {    
    let box = shallow(<ChirpBox currentUser={TEST_USER} />);
    let input = box.find('textarea');
    input.simulate('change', {target:{value: POSTS[0].text}});
    
    let btn = box.find('.btn-primary'); //should be post button
    ...
    expect(chirp).toEqual(expectedObj)
  })
